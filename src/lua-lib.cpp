...
static boost::asio::ip::address_v6 get_address(const char *str) {
  ...
  if (address.is_v4()) {
    return boost::asio::ip::make_address_v6(boost::asio::ip::v4_mapped_t(),
                                            address.to_v4());
  } else {
...
static const struct luaL_Reg filter_metatable[] = {
    {"__gc", safe_call<gc<filter, name_filter>>}, {NULL, NULL}};
...
static const struct luaL_Reg pipeline_metatable[] = {
    ...
    {"start", safe_call<&proxy<decltype(&pipeline::start), &pipeline::start,
                               name_pipeline>::call>},
    {"stop", safe_call<&proxy<decltype(&pipeline::stop), &pipeline::stop,
                              name_pipeline>::call>},
    {NULL, NULL}};
...
static int pipeline_new(lua_State *L) {
  ...
  auto &out =
      *(std::shared_ptr<endpoint> *)luaL_checkudata(L, c, name_endpoint);
  std::vector<std::shared_ptr<filter>> filters(c - 2);
  for (auto i = 2; i < c; ++i) {
    filters[i - 2] =
        *(std::shared_ptr<filter> *)luaL_checkudata(L, i, name_filter);
  }

  new (lua_newuserdata(L, sizeof(std::shared_ptr<pipeline>)))
...
static const struct luaL_Reg endpoint_metatable[] = {
    {"__gc", safe_call<gc<endpoint, name_endpoint>>}, {NULL, NULL}};
...
static int udp_create_channel(lua_State *L) {
  ...
  new (lua_newuserdata(L, sizeof(std::shared_ptr<endpoint>)))
      std::shared_ptr<endpoint>(u->create_channel(peer));
...
static int udp_new(lua_State *L) {
  auto &io_context =
      *(boost::asio::io_context *)lua_touserdata(L, lua_upvalueindex(1));
  switch (lua_gettop(L)) {
  case 0:
    new (lua_newuserdata(L, sizeof(std::shared_ptr<udp>)))
        std::shared_ptr<udp>(new udp(io_context));
    break;
  case 1: {
    auto peer = boost::asio::ip::udp::endpoint(boost::asio::ip::udp::v6(),
                                               (int)lua_tonumber(L, 1));
    new (lua_newuserdata(L, sizeof(std::shared_ptr<udp>)))
        std::shared_ptr<udp>(new udp(io_context, peer));
    break;
  }
  default:
...
static int udp_mux_server_create_channel(lua_State *L) {
  ...
  auto &udp = *reinterpret_cast<std::shared_ptr<udp_mux_server> *>(
      luaL_checkudata(L, 1, name_udp_mux_server));
  new (lua_newuserdata(L, sizeof(std::shared_ptr<endpoint>)))
      std::shared_ptr<endpoint>(udp->create_channel(id));
  luaL_getmetatable(L, name_endpoint);
  lua_setmetatable(L, -2);
  return 1;
}
...
static int udp_mux_client_new(lua_State *L) {
  ...
  case 3: {
    ...
    auto peer =
        boost::asio::ip::udp::endpoint(get_address(peer_address), peer_port);
    new (lua_newuserdata(L, sizeof(std::shared_ptr<endpoint>)))
  ...
  case 5: {
    ...
    auto local =
        boost::asio::ip::udp::endpoint(get_address(local_address), local_port);
    new (lua_newuserdata(L, sizeof(std::shared_ptr<endpoint>)))
        std::shared_ptr<endpoint>(
            new udp_mux_client(io_context, id, peer, local));
    break;
  }
  default:
...
static int hole_open(lua_State *L) {
  ...
  luaL_checkversion(L);
  auto size = (sizeof(hole) / sizeof(hole[0]) - 1) +
              (sizeof(hole_io_object) / sizeof(hole_io_object[0]) - 1);
  lua_createtable(L, 0, size);
  luaL_setfuncs(L, hole, 0);
  lua_pushlightuserdata(L, &io_context);
