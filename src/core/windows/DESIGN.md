...
- **Handle Lifecycle & Cleanup**: Boost.Asio's `windows::object_handle` takes ownership of the wrapped native event handles (`_ReadEvent` and `_WriteEvent`) upon construction/assignment. Closing or resetting the `windows::object_handle` closes the underlying native handle. Therefore, to prevent a double-close crash (`0xC0000008: An invalid handle was specified.`), any manual `CloseHandle` calls must only be performed if the corresponding `windows::object_handle` was never constructed, and the native handles must be set to `nullptr` once the `windows::object_handle` is closed or reset.
