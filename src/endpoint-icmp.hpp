...
class icmp : public std::enable_shared_from_this<icmp> {
	...
	private:
		typedef std::map<boost::asio::ip::icmp::endpoint, std::move_only_function<read_handler>> tm;
		...
		std::queue<std::tuple<boost::asio::ip::icmp::endpoint, boost::asio::const_buffer const, std::move_only_function<write_handler>>> write_queue;
		...
		void write(boost::asio::ip::icmp::endpoint const &peer, boost::asio::const_buffer const &b, std::move_only_function<write_handler> &&handler);
