...
class icmp : public std::enable_shared_from_this<icmp> {
	...
	private:
		typedef std::map<boost::asio::ip::icmp::endpoint, fu2::unique_function<read_handler>> tm;
		...
		std::queue<std::tuple<boost::asio::ip::icmp::endpoint, boost::asio::const_buffer const, fu2::unique_function<write_handler>>> write_queue;
		...
		void read(boost::asio::ip::icmp::endpoint const &peer, fu2::unique_function<read_handler> &&handler);
		void write(boost::asio::ip::icmp::endpoint const &peer, boost::asio::const_buffer const &b, fu2::unique_function<write_handler> &&handler);
