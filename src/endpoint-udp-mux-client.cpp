...
udp_mux_client::udp_mux_client(boost::asio::io_context& io_context, uint8_t id, boost::asio::ip::udp::endpoint peer)
    : socket(io_context), id(id), peer(peer), local(boost::asio::ip::udp::v6(), 0) {}
udp_mux_client::udp_mux_client(boost::asio::io_context& io_context, uint8_t id, boost::asio::ip::udp::endpoint peer,
                               boost::asio::ip::udp::endpoint local)
    : socket(io_context), id(id), peer(peer), local(local) {}
...
void udp_mux_client::async_start(std::move_only_function<event>&& handler) {
  ...
  case none:
    ...
    socket.async_connect(peer,
                         [me = shared_from_this(), handler{std::move(handler)}](const gh::error_code& ec) mutable {
                           if (!ec) {
                             me->state = running;
                           }
...
void udp_mux_client::async_read(std::move_only_function<read_handler>&& handler) {
  ...
  socket.async_receive(buffer, [me = shared_from_this(), p{std::move(p)}, handler{std::move(handler)}](
...
void udp_mux_client::async_write(packet&& p, std::move_only_function<write_handler>&& handler) {
  ...
  socket.async_send(boost::asio::const_buffer{buffer},
